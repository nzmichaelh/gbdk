...
cl_mem::cl_mem(enum mem_class atype, long asize, int awidth):
  cl_base()
{
  int i;
...
void
cl_mem::write(long addr, ulong *val)
{
  class cl_memloc *loc;
...
void
cl_mem::set(long addr, ulong val)
{
  if (addr >= size)
    return;
  if (width <= 8)
...
void
cl_mem::set_bit1(long addr, ulong bits)
{
  if (addr >= size)
    return;
  bits&= mask;
...
void
cl_mem::set_bit0(long addr, ulong bits)
{
  if (addr >= size)
    return;
  bits&= mask;
...
void
cl_mem::dump(long start, long stop, int bpl, class cl_console *con)
{
  int i;
