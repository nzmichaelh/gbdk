...
cl_sim::stop(int reason)
{
  ...
  if (cmd->frozen_console)
	...
	cmd->frozen_console->read_line();
      ...
      cmd->frozen_console->printf("Stop at %06x: (%d) ", uc->PC, reason);
      switch (reason)
	{
	case resHALT:
	...
	default:
      ...
      cmd->frozen_console->printf("F %06x\n", uc->PC); // for sdcdb
      if (cmd->actual_console != cmd->frozen_console)
	cmd->frozen_console->prompt();
...
"info hardware      Status of hardware elements of the CPU",
