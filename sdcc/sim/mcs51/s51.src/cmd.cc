...
static bool
cmd_quit(char *cmd, class cl_uc *uc, class cl_sim *sim)
{
  return(TRUE);
}
...
static bool
cmd_kill(char *cmd, class cl_uc *uc, class cl_sim *sim)
{
  sim->state|= SIM_QUIT;
  return(TRUE);
}
...
static bool
cmd_load(char *cmd, class cl_uc *uc, class cl_sim *sim)
{
  char *name;
  long l;

  if ((name= strtok(NULL, delimiters)) == NULL)
    fprintf(sim->cmd_out(), "File name has not given\n");
  else
    if ((l= uc->read_hex_file(name)) >= 0)
      fprintf(sim->cmd_out(), "%ld bytes read from %s\n", l, name);
  return(FALSE);
}
...
static bool
cmd_download(char *cmd, class cl_uc *uc, class cl_sim *sim)
{
  long l;

  if ((l= uc->read_hex_file(NULL)) >= 0)
    fprintf(sim->cmd_out(), "%ld bytes read\n", l);
  //while ((c= getc(uc->cmd_in)) != '\n') ;
  return(FALSE);
}
...
static bool
cmd_reset(char *cmd, class cl_uc *uc, class cl_sim *sim)
{
  uc->reset();
  return(FALSE);
}
...
struct cmd_entry cmd_table[]=
{
  ...
  {"dl", cmd_download, FALSE,
   "dl,do              Download (intel hex) data"},
  {"do", cmd_download, FALSE, NULL},
