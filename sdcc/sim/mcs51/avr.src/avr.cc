...
{
  uchar data, sreg= get_mem(MEM_IRAM, SREG);
  uint x, y, z;
  class cl_mem *m;
  ...
  con->printf("SP  = 0x%06x\n",
	      get_mem(MEM_IRAM, SPH)*256+get_mem(MEM_IRAM, SPL));

  x= get_mem(MEM_IRAM, XH)*256 + get_mem(MEM_IRAM, XL);
  data= get_mem(MEM_IRAM, x);
  con->printf("X= 0x%04x [X]= 0x%02x %3d %c  ", x,
	      data, data, isprint(data)?data:'.');
  y= get_mem(MEM_IRAM, YH)*256 + get_mem(MEM_IRAM, YL);
  data= get_mem(MEM_IRAM, y);
  con->printf("Y= 0x%04x [Y]= 0x%02x %3d %c  ", x,
	      data, data, isprint(data)?data:'.');
  z= get_mem(MEM_IRAM, ZH)*256 + get_mem(MEM_IRAM, ZL);
  data= get_mem(MEM_IRAM, z);
...
cl_avr::exec_inst(void)
{
  ...
  switch (code & 0xf000)
	...
	case 0x8200: return(st_Z_q_Rr(code));
	...
	case 0x9000:
	  switch (code & 0xfe0f)
	    ...
	    case 0x920a: return(st$Y_Rr(code));
	...
	case 0x9800:
	  switch (code & 0xff00)
	    ...
	    case 0x9900: return(sbic_A_b(code));
	    case 0x9a00: return(sbi_A_b(code));
	    case 0x9b00: return(sbis_A_b(code));
	    }
	...
	case 0xa200: return(st_Z_q_Rr(code));
